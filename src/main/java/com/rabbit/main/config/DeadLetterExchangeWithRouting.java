...
@Configuration
public class DeadLetterExchangeWithRouting {
	@Autowired
	...
	Queue createDeadLetterRouterQueue() {
		return QueueBuilder.durable(QUEUE_NAME).ttl(5000).deadLetterExchange("")// "" means default-exchange
				.deadLetterRoutingKey(DEFAULT_QUEUE).build();
	}
